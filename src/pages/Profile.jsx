...
const ProfileNew = () => {
  ...
  const galleryPhotoInputRef = React.useRef(null);
  ...
  const handleGalleryPhotoUpload = async (e) => {
    ...
    if (file && photos.length < 6) {
      ...
      try {
        ...
        // Only set as profile photo if no profile photo exists
  ...
  const handleAddPhoto = () => {
    if (newPhotoUrl.trim() && photos.length < 6) {
      const newPhotos = [...photos, newPhotoUrl.trim()];
      setPhotos(newPhotos);
      ...
      <main className="flex-1 py-8">
        <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
            {/* Profile Header */}
            <div className="relative h-48 bg-gradient-to-br from-pink-500 via-purple-500 to-purple-600">
            ...
            <div className="pt-20 px-8 pb-8">
              {/* Profile Photo Upload (Edit Mode) */}
              {isEditing && (
                <div className="mb-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
                  ...
                  <button
                    ...
                    onClick={() => profilePhotoInputRef.current?.click()}
                    disabled={uploading}
                    className="flex items-center gap-2 px-4 py-2 bg-white border-2 border-primary-500 text-primary-600 rounded-lg font-semibold hover:bg-primary-50 transition disabled:opacity-50"
                  >
                    <Camera className="w-5 h-5" />
                    {uploading ? 'Uploading...' : 'Upload Profile Photo'}
                  </button>
                  <p className="text-xs text-gray-500 mt-2">
                    Click to upload your main profile photo (Max 5MB). You can also click the profile circle above or set from gallery below.
                  </p>
                </div>
              )}
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                    ...
                    <div key={index} className="relative group aspect-square rounded-lg overflow-hidden border-2 border-gray-200">
                      ...
                      {isEditing && (
                        <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-2">
                          ...
                          >
                            <Heart className={`w-5 h-5 ${formData.profile_photo === photo ? 'text-pink-500 fill-current' : 'text-gray-600'}`} />
                          </button>
                          <button
                            ...
                            title="Remove photo"
                          >
                            <Trash2 className="w-5 h-5 text-pink-500" />
                          </button>
                        </div>
                      )}
                  ...
                  {isEditing && photos.length < 6 && (
                    <div 
                      ...
                      className="aspect-square rounded-lg border-2 border-dashed border-gray-300 flex items-center justify-center bg-gray-50 hover:bg-gray-100 cursor-pointer transition"
                    >
